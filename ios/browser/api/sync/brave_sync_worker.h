...
class BraveSyncWorker : public syncer::SyncServiceObserver {
 public:
  ...
  brave_sync::QrCodeDataValidationResult GetQrCodeValidationResult(const std::string json);
  brave_sync::WordsValidationStatus GetWordsValidationResult(const std::string time_limited_words);
  std::string GetWordsFromTimeLimitedWords(const std::string& time_limited_words);
 ...
 private:
  ...
  base::ScopedMultiSourceObservation<syncer::SyncService, syncer::SyncServiceObserver>
      sync_service_observer_{this};
