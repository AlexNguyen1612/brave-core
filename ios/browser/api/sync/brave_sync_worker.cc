...
std::string BraveSyncWorker::GetQrCodeJsonFromHexSeed(const std::string& hex_seed) {
  DCHECK(!hex_seed.empty());
  return brave_sync::QrCodeData::CreateWithActualDate(hex_seed)->ToJson();
}

brave_sync::QrCodeDataValidationResult BraveSyncWorker::GetQrCodeValidationResult(const std::string json) {
  DCHECK(!json.empty());
  return brave_sync::QrCodeDataValidator::ValidateQrDataJson(json);
}

brave_sync::WordsValidationStatus BraveSyncWorker::GetWordsValidationResult(const std::string time_limited_words) {
  DCHECK(!time_limited_words.empty());
  return brave_sync::TimeLimitedWords::Parse(time_limited_words).status;
}

std::string BraveSyncWorker::GetWordsFromTimeLimitedWords(const std::string& time_limited_words) {
  DCHECK(!time_limited_words.empty());
  auto words_with_status = brave_sync::TimeLimitedWords::Parse(time_limited_words);
  DCHECK_EQ(words_with_status.status, brave_sync::WordsValidationStatus::kValid);
  DCHECK(words_with_status.pure_words.has_value());
  return words_with_status.pure_words.value();
}

std::string BraveSyncWorker::GetTimeLimitedWordsFromWords(const std::string& words) {
  DCHECK(!words.empty());
...
std::string BraveSyncWorker::GetHexSeedFromQrCodeJson(const std::string& json) {
  DCHECK(!json.empty());
  std::unique_ptr<brave_sync::QrCodeData> qr_data = brave_sync::QrCodeData::FromJson(json);
  if (qr_data) {
